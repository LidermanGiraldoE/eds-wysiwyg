...
export async function performMonolithGraphQLQuery(query, variables, GET = true, USE_TOKEN = false) {
  ...
  const storeViewCode = await getConfigValue('commerce.headers.cs.Magento-Store-View-Code') || 'storeview_ulta';
  ...
  console.log('Header Commerce: ', JSON.stringify(headers, null, 2));
...
export function renderPrice(product, format, html = (strings, ...values) => strings.reduce((result, string, i) => result + string + (values[i] || ''), ''), Fragment = null) {
  ...
  if (product.price) {
    ...
    return html`<${Fragment}>
      <span class="price-final special">${format(final.amount.value)}</span><span class="price-regular">${format(regular.amount.value)}</span>
    </${Fragment}>`;
  }
  ...
  if (product.priceRange) {
    ...
    if (finalMin.amount.value !== finalMax.amount.value) {
      ...
      <div class="price-range">
        ...
        <span class="price-from">Desde ${format(finalMin.amount.value)}</span>
      </div>`;
    }

    if (finalMin.amount.value !== regularMin.amount.value) {
      return html`<${Fragment}>
      <span class="price-final special">${format(finalMin.amount.value)}</span><span class="price-regular">${format(regularMin.amount.value)}</span>
    </${Fragment}>`;
    }
