...
async function loadCategory(state) {
  try {
    console.log("loadCategory", state);
    ...
    // Solo filtrar por categoryPath, no por id
    if (state.type === "category" && state.category.urlPath) {
      variables.filter = variables.filter || [];
      // Solo se incluye el filtro por categoryPath
...
function parseQueryParams() {
  ...
  console.log('params Oscar', params);
  const newState = {
    filters: {
      inStock: ['true'],
    },
...
export async function preloadCategory(category) {
  console.log('preloadCategory', category);
...
function getCategoryInfo() {
  ...
  // Verificar que la ruta contiene "/categories/"
  const pathParts = path.split("/categories/")[1];

  if (!pathParts) {
    console.error("La URL no contiene los parámetros esperados.");
    return null; // Retornamos null si la ruta no es válida
  }

  // El urlPath es todo lo que sigue después de "/categories/"
  const urlpath = pathParts; // Esto ya es todo lo que queda después de "/categories/"

  // Retornar un objeto con el urlPath y un id dummy
  return { id: "dummy", urlpath };
}

export default async function decorate(block) {
  const config = getCategoryInfo();
  console.log('decorate', config);
