...
const jsonURL =
  'https://author-p147931-e1511083.adobeaemcloud.com/content/dam/ulta/json/products.json';
...
export default async function decorate(block) {
  ...
  if (dataTags.length > 0) {
    selectedTags = dataTags[0].textContent
      .trim()
      .split(',')
      .map((tag) => tag.trim());
  }
  ...
  swiperWrapper.classList.add(
    'ulta-products-carousel-wrapper',
    'swiper-wrapper'
  );
      ...
      <div class="ulta-product">
        <a href="${linkPdp}" class="ulta-product-link">
            <div class="ulta-product-image">
              ...
              ${
                badge
                  ? `<img src="${badge}" alt="Badge" class="ulta-product-badge">`
                  : ''
              }
              ${
                wishlist
                  ? `<a href="${wishlistLink}" class="ulta-product-wishlist-button ${
                      wishlistAdded ? 'remove' : ''
                    }" ></a>`
                  : ''
              }
              ${
                tag.length > 0
                  ...
                  <div class="ulta-tag-container">
                      ${tag
                        .map((t) => `<div class="ulta-tag">${t}</div>`)
                        .join('')}
                  </div>
              `
                  : ''
        ...
        <div class="ulta-product-info">
            ...
            ${
              ranking
                ? `<div class="ulta-product-stars-wrapper">
        ...
        ${
          blockButtonText
            ? `<a class="ulta-product-button" href="${addToCartLink}">${blockButtonText}</a>`
            : ''
        }
      </div>
      `;
